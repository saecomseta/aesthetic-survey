  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 w-full">
      <div className={`flex-[2] flex flex-col border border-gray-200 rounded-lg overflow-hidden focus-within:ring-2 ${ringColor} transition-all`}>
        <div className="flex bg-white/5 border-b border-gray-200 overflow-x-auto scrollbar-hide">
          {tabs.map(t => (
            <button 
              key={t.key}
              type="button"
              onClick={() => setActiveTab(t.key)}
              className={`flex-1 px-2 py-2.5 text-xs font-bold whitespace-nowrap transition-colors border-b-2 ${activeTab === t.key ? `bg-transparent ${activeColor}` : 'border-transparent text-gray-400 hover:bg-white/5 hover:text-brand-text/90'}`}
            >
              {t.label}
            </button>
          ))}
        ...
        <textarea
          ...
          className="w-full p-5 outline-none resize-none h-48 text-sm bg-transparent font-medium text-brand-text leading-relaxed"
      ...
      <div className={`flex-[1] bg-${theme === 'blue' ? 'blue' : 'purple'}-50 border border-${theme === 'blue' ? 'blue' : 'purple'}-100 rounded-lg p-5 flex flex-col gap-4 min-w-[260px]`}>
        ...
        <ul className="text-xs text-brand-text/90 space-y-4 leading-relaxed">
          <li>
            <code className="text-primary-600 font-bold bg-transparent px-1.5 py-0.5 rounded border border-gray-200 shadow-sm">{`{{name}}`}</code>
            <p className="mt-1.5 text-brand-text/70">환자의 이름이 자동 치환됩니다. (예: 홍길동)</p>
          </li>
          <li>
            <code className="text-primary-600 font-bold bg-transparent px-1.5 py-0.5 rounded border border-gray-200 shadow-sm">{`{{sector:변수명}}`}</code>
            <p className="mt-1.5 text-brand-text/70">조건이 일치하여 도출된 해당 섹터의 <strong>개별 소견</strong>이 1:1로 정확히 치환됩니다. 점수 미달 시 해당 영역은 공백으로 자동 소거됩니다.</p>
          </li>
          <li>
            <code className="text-primary-600 font-bold bg-transparent px-1.5 py-0.5 rounded border border-gray-200 shadow-sm">{`{{extra_advice}}`}</code>
            <p className="mt-1.5 text-brand-text/70">선택된 문항 옵션들에 지정된 <strong>추가 소견</strong> 문구들이 리스트업됩니다. 중복 내용은 하나로 제거됩니다.</p>
          </li>
        </ul>
      </div>
    </div>
