...
import { Plus, Trash2, ArrowLeft, Save, GripVertical, Settings2, FileText, Target, LayoutGrid } from 'lucide-react'
...
export default function NewSurveyPage() {
  ...
  const handleSave = async () => {
    ...
    if (sectors.length === 0) return alert('최소 하나 이상의 섹터를 추가해주세요.')
    for (const s of sectors) {
      ...
      for (const sc of s.scripts) {
        if (!sc.content.trim()) return alert('모든 스크립트 작성 칸의 내용을 입력해주세요.')
      }
    }
    ...
    try {
      ...
      router.push(`/admin/surveys/${survey.id}?created=true`)
    } catch (e) {
      console.error(e)
      alert('설문지 저장 중 시스템 오류가 발생했습니다.')
      setSaving(false)
    }
  }
    ...
    <AdminGuard requireSuperAdmin={true}>
      <main className="min-h-screen w-full bg-[#7A5B44] font-sans">
        ...
        <div className="flex items-center justify-between mb-8">
          <Link href="/admin" className="text-brand-text/70 hover:text-primary-700 font-medium flex items-center gap-2 transition-colors">
            <ArrowLeft className="w-5 h-5"/> 대시보드로 돌아가기
          </Link>
          <button 
            onClick={handleSave}
            disabled={saving}
            className="px-6 py-2.5 glass-button text-white rounded-lg hover:bg-primary-700 transition shadow flex items-center gap-2 font-medium disabled:opacity-70"
          >
            {saving ? '저장 처리 중...' : <><Save className="w-5 h-5"/> 저장 및 즉시 배포</>}
          </button>
        </div>

        <div className="bg-transparent rounded-2xl shadow-sm border border-beige-200 p-8 mb-8 sticky top-4 z-10">
          <input
            type="text"
            placeholder="설문지 전체 제목 (예: VIP 맞춤형 피부 정밀 진단표)"
            className="w-full text-3xl font-light text-primary-700 placeholder:text-gray-300 border-none outline-none bg-transparent"
            value={title}
            onChange={e => setTitle(e.target.value)}
          />
        </div>

        <div className="bg-transparent rounded-3xl shadow-sm border border-blue-200 p-8 mb-8">
          ...
          <p className="text-sm text-brand-text/70 mb-6">환자의 최종 결과를 조립할 때 뼈대가 되는 마스터 템플릿입니다. 이름 부분은 <code>{`{{name}}`}</code>, 섹터 교체 부분은 <code>{`{{sector_advice}}`}</code>, 추가 문구는 <code>{`{{extra_advice}}`}</code>로 치환됩니다.</p>
          <TargetScriptEditor 
            theme="blue"
        ...
        <div className="bg-transparent rounded-3xl border-2 border-primary-200 p-8 mb-8 shadow-sm">
          ...
          <p className="text-sm text-brand-text/70 mb-6">특정 고유 변수명들을 가진 스크립트들이 동시에 만족되었을 때 출력될 특별한 문구를 정의합니다.</p>
          
          <div className="space-y-4">
            {combinations.map((comb, cIdx) => (
              <div key={comb.id} className="bg-transparent rounded-2xl p-5 border border-beige-200 relative group">
                <div className="flex flex-col md:flex-row gap-4">
                  <div className="md:w-1/3">
                    ...
                    <input 
                      ...
                      className="w-full bg-transparent border border-beige-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary-300 font-bold"
                  ...
                  <div className="flex-1">
                    ...
                    <textarea 
                      rows={2}
                      className="w-full bg-transparent border border-beige-200 rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-primary-300"
                ...
                <button 
                  ...
                  className="absolute -top-2 -right-2 bg-transparent text-red-400 hover:text-red-500 p-1.5 rounded-full border border-red-100 shadow-sm transition-colors opacity-0 group-hover:opacity-100"
                >
        ...
        <div className="space-y-12">
          {sectors.map((sector, sIndex) => (
            <div key={sector.id} className="bg-transparent/50 rounded-3xl border-2 border-primary-200 shadow-sm overflow-hidden">
              <div className="glass-button p-6 flex flex-col md:flex-row gap-4 justify-between items-start md:items-center text-white">
                ...
                <div className="flex items-center gap-4 border-t border-primary-500 pt-4 md:border-t-0 md:pt-0 w-full md:w-auto mt-2 md:mt-0">
                  ...
                  <button onClick={() => removeSector(sIndex)} className="p-2 hover:bg-red-900/300 rounded-lg transition-colors bg-black/10 shrink-0">
              ...
              <div className="p-6 space-y-8">
                {/* Sector Config */}
                <div className="bg-transparent rounded-xl p-5 border border-beige-200 shadow-sm flex flex-wrap gap-4 items-center">
                  ...
                  <label className="flex flex-col text-sm text-brand-text/80">섹터 기본 시작 점수
                  ...
                  <label className="flex flex-col text-sm text-brand-text/80">20대 이하 가중치(곱)
                  ...
                  <label className="flex flex-col text-sm text-brand-text/80">30대 관리군 가중치
                  ...
                  <label className="flex flex-col text-sm text-brand-text/80">40대 이상 가중치
                ...
                {/* Questions */}
                <div className="space-y-4">
                  <h4 className="font-medium text-brand-text flex items-center gap-2"><Target className="w-4 h-4 text-primary-500"/> 설문 문항 구성하기</h4>
                  {sector.questions.map((q, qIndex) => (
                    <div key={q.id} className={`bg-transparent rounded-xl shadow-sm p-5 flex gap-4 relative group transition-all ${q.parent_id ? 'border-primary-400 ml-8 md:ml-12 border-l-4 border-y border-r' : 'border border-beige-200'}`}>
                      {q.parent_id && (
                        <div className="absolute -left-10 md:-left-12 top-6 flex items-center gap-2 text-primary-500 font-bold whitespace-nowrap">
                          <span className="bg-white/5 px-2 py-1 rounded text-xs border border-primary-200">'{q.trigger_value}' 선택 시</span>
                      ...
                      <div className="flex-1 space-y-4">
                        <div className="flex flex-col md:flex-row gap-4 items-start">
                          <input
                            ...
                            className="flex-1 text-base font-medium bg-white/5 border border-gray-100 rounded-lg px-4 py-2 outline-none focus:ring-2 focus:ring-primary-300 transition w-full"
                            value={q.content}
                        ...
                        {q.type === 'o_x' && (
                          <div className="bg-white/5 p-3 rounded-lg flex flex-col gap-4">
                            <div className="flex flex-col md:flex-row md:items-center gap-3">
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                />
                                <span className="text-xs text-brand-text/70">('X' 선택 시에는 0점)</span>
                              </div>
                            </div>
                            <div className="flex flex-col gap-2 bg-transparent p-3 rounded-lg border border-primary-100">
                              <span className="text-xs font-bold text-brand-text/80 text-left">선택 시 누적될 추가 소견 (<code>{`{{extra_advice}}`}</code> 영역에 삽입)</span>
                              <input type="text" placeholder="'O' 선택 시 추가 문구" className="text-sm px-3 py-2 border border-gray-200 bg-white/5 rounded" value={q.options_extra_advices['O'] || ''} onChange={e => updateQuestion(sIndex, qIndex, {options_extra_advices: {...q.options_extra_advices, O: e.target.value}})} />
                              <input type="text" placeholder="'X' 선택 시 추가 문구" className="text-sm px-3 py-2 border border-gray-200 bg-white/5 rounded" value={q.options_extra_advices['X'] || ''} onChange={e => updateQuestion(sIndex, qIndex, {options_extra_advices: {...q.options_extra_advices, X: e.target.value}})} />
                            </div>
                            {!q.parent_id && (
                              <button onClick={() => addSubQuestion(sIndex, qIndex, 'O')} className="text-primary-600 text-xs font-bold hover:underline px-2 py-1 bg-transparent rounded border border-primary-200 shadow-sm transition hover:glass-button hover:text-white w-fit">+ 'O' 선택 시 구체적으로 물어볼 하위 질문 추가하기</button>
                            )}
                          </div>
                        )}
                        ...
                        {q.type === 'multiple_choice' && (
                          <div className="space-y-4 pl-2 border-l-2 border-beige-300 ml-1">
                            {q.options.map((opt, oIndex) => {
                              ...
                              return (
                                <div key={oIndex} className="flex flex-col gap-2 bg-white/5 p-3 rounded-xl border border-gray-200">
                                  <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                                    <div className="hidden sm:block w-3 h-3 border border-gray-400 flex-shrink-0 rounded-full"/>
                                    <input
                                      type="text"
                                      placeholder={`선택지 텍스트`}
                                      className="flex-[2] bg-transparent border border-gray-300 px-3 py-1.5 rounded-lg outline-none focus:border-primary-500 text-sm"
                                      value={label}
                                      onChange={e => {
                                    ...
                                    <div className="flex items-center gap-2">
                                      <input
                                        type="number"
                                        placeholder={`배점`}
                                        className="w-20 bg-transparent border border-gray-300 rounded-lg px-2 py-1.5 outline-none focus:border-primary-500 text-sm transition text-center"
                                      ...
                                      />
                                      <span className="text-xs text-brand-text/70 font-medium whitespace-nowrap">점</span>
                                      <button onClick={() => {
                                        const newOpts = q.options.filter((_, i) => i !== oIndex)
                                        updateQuestion(sIndex, qIndex, { options: newOpts })
                                      }} className="text-gray-400 hover:text-red-500 ml-2 bg-transparent p-1 rounded shadow-sm border border-gray-200"><Trash2 className="w-4 h-4"/></button>
                                    </div>
                                  </div>
                                  <input type="text" placeholder={`선택 시 누적될 추가 소견 ({{extra_advice}})`} className="text-sm px-3 py-1.5 border border-primary-200 bg-transparent rounded-lg w-full outline-none focus:ring-1 focus:ring-primary-400 focus:border-primary-400 transition" value={q.options_extra_advices[label] || ''} onChange={e => updateQuestion(sIndex, qIndex, {options_extra_advices: {...q.options_extra_advices, [label]: e.target.value}})} />
                                </div>
                              )
                            })}
                            <button onClick={() => updateQuestion(sIndex, qIndex, { options: [...q.options, `새 선택지 ${q.options.length + 1}||0`] })} className="text-primary-600 text-xs font-bold hover:underline mt-2 inline-block px-3 py-2 bg-white/5 rounded-lg border border-primary-100 transition hover:bg-primary-100">+ 세부 항목 및 배점 추가</button>
                          </div>
                        )}
                              ...
                              <div key={oIndex} className="flex flex-col gap-2 bg-white/5 p-3 rounded-xl border border-gray-200">
                                <div className="flex items-center gap-2">
                                  ...
                                  <input
                                    ...
                                    className="flex-1 bg-transparent border border-gray-300 rounded-lg px-3 py-1.5 outline-none focus:border-primary-500 text-sm"
                                    value={opt}
                                    onChange={e => {
                                      const newOpts = [...q.options]
                                  ...
                                  <button onClick={() => {
                                      const newOpts = q.options.filter((_, i) => i !== oIndex)
                                      updateQuestion(sIndex, qIndex, { options: newOpts })
                                    }} className="text-gray-400 hover:text-red-500 bg-transparent p-1 rounded shadow-sm border border-gray-200"><Trash2 className="w-4 h-4"/></button>
                                </div>
                                <input type="text" placeholder={`선택 시 누적될 추가 소견 ({{extra_advice}})`} className="text-sm px-3 py-1.5 border border-primary-200 bg-transparent rounded-lg w-full outline-none focus:ring-1 focus:ring-primary-400 focus:border-primary-400 transition" value={q.options_extra_advices[opt] || ''} onChange={e => updateQuestion(sIndex, qIndex, {options_extra_advices: {...q.options_extra_advices, [opt]: e.target.value}})} />
                              </div>
                            ))}
                            <button onClick={() => updateQuestion(sIndex, qIndex, { options: [...q.options, `옵션 ${q.options.length + 1}`] })} className="text-primary-600 text-xs font-bold hover:underline mt-2 inline-block px-3 py-2 bg-white/5 rounded-lg border border-primary-100 transition hover:bg-primary-100">+ 선택지 추가하기</button>
                          </div>
                        )}
                        <div className="flex justify-between items-center text-sm pt-2 border-t border-gray-50">
                          ...
                          <label className="flex items-center gap-2 cursor-pointer font-medium text-brand-text/80">
                  ...
                  <div className="flex gap-2 text-sm flex-wrap">
                    <button onClick={() => addQuestion(sIndex, 'multiple_choice')} className="px-3 py-1.5 glass-button border border-primary-700 text-white rounded-lg hover:bg-primary-700 transition shadow-sm font-medium">+ 다항 객관식 추가</button>
                    <button onClick={() => addQuestion(sIndex, 'o_x')} className="px-3 py-1.5 bg-transparent border border-primary-200 text-primary-700 rounded-lg hover:bg-white/5 transition shadow-sm font-medium">+ O/X 점수 수집형 문항 추가</button>
                    <button onClick={() => addQuestion(sIndex, 'text')} className="px-3 py-1.5 bg-transparent border border-gray-200 text-brand-text/80 rounded-lg hover:bg-white/5 transition shadow-sm">+ 일반 문항(입력/선택) 추가</button>
                  </div>
                </div>
                ...
                {!sector.is_common && (
                  <div className="space-y-4 pt-4 border-t border-beige-200">
                    <h4 className="font-medium text-brand-text flex items-center gap-2"><FileText className="w-4 h-4 text-primary-500"/> 해당 섹터 통합 점수 구간별 자동 매칭 타입 작성</h4>
                    {sector.scripts.map((sc, scIndex) => (
                      <div key={sc.id} className="bg-transparent rounded-xl shadow-sm border border-blue-100 p-4 relative">
                        <div className="flex flex-col xl:flex-row gap-4">
                          <div className="flex flex-col items-start gap-1 xl:w-64 shrink-0 mt-1">
                            ...
                            <div className="flex items-center gap-2 w-full bg-blue-50 p-2 rounded-lg border border-blue-100 mt-1">
                              <input type="number" placeholder="최소 점수" value={sc.min_score} onChange={e=>updateScript(sIndex, scIndex, {min_score: Number(e.target.value)})} className="w-16 text-center px-1 py-1 rounded bg-transparent border border-blue-200 outline-none focus:ring-2 focus:ring-blue-400"/>
                              <span className="text-blue-400 font-bold">~</span>
                              <input type="number" placeholder="최대 점수" value={sc.max_score} onChange={e=>updateScript(sIndex, scIndex, {max_score: Number(e.target.value)})} className="w-16 text-center px-1 py-1 rounded bg-transparent border border-blue-200 outline-none focus:ring-2 focus:ring-blue-400"/>
                            </div>
                            <div className="mt-4 w-full">
                              ...
                              <input 
                                ...
                                className="w-full mt-1 bg-transparent border border-beige-200 rounded-lg px-3 py-1.5 text-sm font-bold text-primary-700 outline-none focus:ring-2 focus:ring-primary-300"
                              />
                            </div>
                          </div>
                          <div className="flex-1 space-y-1 w-full">
                            <span className="text-xs font-bold text-brand-text/80 px-1 mb-2 block">치환용 단일 진단 소견 (<code>{`{{sector_advice}}`}</code> 영역에 삽입됨)</span>
                            <textarea 
                              rows={3}
                              placeholder="위 구간 점수를 달성했을 때 전역 마스터 템플릿의 {{sector_advice}}에 삽입될 진단 소견 텍스트를 입력하세요."
                              className="w-full text-sm font-medium bg-transparent border border-blue-200 rounded-lg p-3 outline-none focus:ring-2 focus:ring-blue-300 resize-y"
                              value={sc.content}
                              onChange={e => updateScript(sIndex, scIndex, {content: e.target.value})}
                            />
                          </div>
                        </div>
                        <button onClick={()=>removeScript(sIndex, scIndex)} className="absolute top-2 right-2 text-gray-300 hover:text-red-500 bg-transparent shadow-sm rounded-full p-1 border border-gray-100"><Trash2 className="w-4 h-4"/></button>
                      </div>
                    ))}
          ...
          <button 
            onClick={addSector}
            className="w-full py-6 border-2 border-dashed border-primary-300 text-primary-600 rounded-2xl hover:bg-white/5 hover:border-primary-400 transition-colors font-medium text-lg flex justify-center items-center gap-2"
          >
