    ...
    <AdminGuard requireSuperAdmin={true}>
      <div className="max-w-4xl mx-auto p-4 lg:p-10 pb-32">
        <div className="flex items-center justify-between mb-8">
          <Link href="/admin" className="text-gray-500 hover:text-primary-700 font-medium flex items-center gap-2 transition-colors">
            <ArrowLeft className="w-5 h-5"/> 대시보드로 돌아가기
          </Link>
          <button 
            onClick={handleSave}
            disabled={saving}
            className="px-6 py-2.5 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition shadow flex items-center gap-2 font-medium disabled:opacity-70"
          >
            {saving ? '저장 처리 중...' : <><Save className="w-5 h-5"/> 저장 및 즉시 배포</>}
          </button>
        </div>

        <div className="bg-white rounded-2xl shadow-sm border border-beige-200 p-8 mb-8 sticky top-4 z-10">
          <input
            type="text"
        ...
        <div className="bg-white rounded-3xl border-2 border-primary-200 p-8 mb-8 shadow-sm">
          ...
          {sectors.map((sector, sIndex) => (
            <div key={sector.id} className="bg-beige-50/50 rounded-3xl border-2 border-primary-200 shadow-sm overflow-hidden">
              ...
              <div className="p-6 space-y-8">
                ...
                {/* Questions */}
                <div className="space-y-4">
                  <h4 className="font-medium text-gray-800 flex items-center gap-2"><Target className="w-4 h-4 text-primary-500"/> 설문 문항 구성하기</h4>
